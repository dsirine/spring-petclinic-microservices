    ...
    stage('Deploy to AWS') {
      steps {
        withCredentials([[
          ...
          sh 'eksctl create cluster --name petclinicproject --version 1.25 --region eu-west-3 --nodegroup-name standard-workers --node-type t3.micro --nodes 4 --nodes-min 4 --nodes-max 6'
        }
      }
    }
